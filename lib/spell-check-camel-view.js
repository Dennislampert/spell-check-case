...
export default class SpellCheckCamelView {
  ...
  constructor(serializedState) {
    ...
    this.message.textContent = 'Spell check is activated';
  ...
  setMessage(misspelledWords, editor) {
    this.removeMarkers();
    // som saadsasdasdsaddasdadasdasda asdasd asd ad asd asd as sadad d  d d d d
    const me = this;
    if (misspelledWords && misspelledWords.length) {
      markerLayer = editor.addMarkerLayer();
      this.markerLayers.push(markerLayer);
      misspelledWords.forEach((word) => {
        const reg = word[0] === word[0].toUpperCase() ? word+'(?!=*[a-z])': '\\b'+word+'(?!=*[a-z])';
        editor.scanInBufferRange(new RegExp(reg, 'g'), [[0, 0], editor.getEofBufferPosition()], function (result) {
