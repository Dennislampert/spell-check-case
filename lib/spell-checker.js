...
export default {
  ...
  activate(state) {
    ...
    console.log("activate and watching");
    let self = this;
    ...
    // atom.workspace.onDidStopChanging((e)=>{
    //     console.log("stop changing");
    // })
  ...
  triggerSpellcheck(instance = null) {
    let self = instance ? instance : this;
    //const me = self;
    console.log('triggerSpellcheck', self);
    if (!self.activated) {
        console.log('self not activated', self);
      return;
    }
    //const me = this;
    try {
      //const currentFileText = atom.workspace.getActivePaneItem().buffer.cachedText;
      const currentFileText = atom.workspace.getActivePaneItem().buffer.getText();
      if (!currentFileText) {
        console.log('currentFileText not exist', atom.workspace.getActivePaneItem());
        return;
      }
      console.log('currentFileText exists here');
      ...
      if (missSpelled.length) {
        self.statusBarElement.updateCount(missSpelled.length);
        self.spellCheckCamelView.setMessage(missSpelled, atom.workspace.getActiveTextEditor());
      } else {
        self.spellCheckCamelView.setMessage(missSpelled, atom.workspace.getActiveTextEditor());
        self.statusBarElement.updateCount(0);
      }
    } catch(r) {
      //console.log('Error', r);
      // die quiet
